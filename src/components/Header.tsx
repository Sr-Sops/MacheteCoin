...
export default function Header({ twitterUrl, telegramUrl, discordUrl, instagramUrl, facebookUrl, tiktokUrl, youtubeUrl, poocoinUrl }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.error('Error fetching settings in Header', e);
      }
    };
    loadData();
  ...
  const tUrl = twitterUrl || settings?.twitter_url;
  const tgUrl = telegramUrl || settings?.telegram_url;
  const dUrl = discordUrl || settings?.discord_url;
  const iUrl = instagramUrl || settings?.instagram_url;
  const fUrl = facebookUrl || settings?.facebook_url;
  const tkUrl = tiktokUrl || settings?.tiktok_url;
  const yUrl = youtubeUrl || settings?.youtube_url;
  const pUrl = poocoinUrl || settings?.poocoin_url;
  ...
  return (
    <header style={{
      ...
      background: 'linear-gradient(90deg, #0a2d1a 0%, #050a07 35%)',
      ...
      <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'none', alignItems: 'center', gap: '1rem' }} className="md-flex">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginRight: '0.5rem' }}>
            {tUrl && (
              <a href={tUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
              </a>
            )}
            {tgUrl && (
              <a href={tgUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
            ...
            {dUrl && (
              <a href={dUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
            ...
            {iUrl && (
              <a href={iUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
            ...
            {fUrl && (
              <a href={fUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
            ...
            {tkUrl && (
              <a href={tkUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
            ...
            {yUrl && (
              <a href={yUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
                <svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
              </a>
            )}
            {pUrl && (
              <a href={pUrl} title="PooCoin" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-secondary)', transition: 'color 0.2s' }} className="social-hover">
                <LineChart size={20} />
              </a>
            )}
          </div>
      ...
      {mobileMenuOpen && (
        <div style={{
          ...
          <div style={{ display: 'flex', gap: '1.25rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            {tUrl && (
              <a href={tUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {tgUrl && (
              <a href={tgUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {dUrl && (
              <a href={dUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {iUrl && (
              <a href={iUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
                <svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
              </a>
            )}
            {fUrl && (
              <a href={fUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {tkUrl && (
              <a href={tkUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {yUrl && (
              <a href={yUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
            ...
            {pUrl && (
              <a href={pUrl} title="PooCoin" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--color-gold)' }}>
                <LineChart size={24} />
              </a>
            )}
          </div>
